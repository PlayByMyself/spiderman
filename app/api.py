...
def check_cron_expression(expression: str):
    assert CronValidator.parse(expression) is None, f"Invalid cron expression {expression}"
    return expression
...
TriggerIn = Annotated[Union[DateTriggerIn, IntervalTriggerIn, CronTriggerIn], Field(discriminator="triger")]
...
def get_jobs():
    ...
    return [{"job_id": job.id,"kwargs": job.kwargs, "trigger": repr(job.trigger)} for job in jobs]

@job_router.post("")
def add_job(spider: SpiderIn, trigger: TriggerIn):
    spider_kwargs = spider.model_dump()
    trigger_kwargs = trigger.model_dump(exclude={"triger",})
    job = scheduler.add_job(
        run_spider,
        kwargs=spider_kwargs,
        trigger=trigger.triger,
        **trigger_kwargs
    )
    return {"job_id": job.id}
